...
class TestMotorEncoder(TestCase):
    ...
    def test_angle_wrap_wheel_diff(self):
        ...
        for test_answer, test_diff in zip(test_answers, test_diffs):
            self.assertAlmostEqual(test_answer, er.angle_wrap_wheel_diff(test_diff), delta=1e-4)
    ...
    def test_angle_diffs(self):
        ...
        for test_pose, test_answer in zip(test_encoder_poses_single, test_answers_single):
            test_answers, test_poses = np.ones(2) * test_answer, np.ones(2) * test_pose
            test_results = er.get_angle_diffs(test_poses)
            #TODO Remember to remove
            print(f'Rico test results: {test_results}, test answers: {test_answers}')
            self.assertTrue(np.allclose(test_answers, test_results, rtol=1e-5, atol=1e-8))
