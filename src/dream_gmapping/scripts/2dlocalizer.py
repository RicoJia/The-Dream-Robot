...
from localizer_2d_utils import get_vector_1_pixel_away_vectorized, map_pixel_to_matrix, create_mask, MapValue

SEARCH_GRID_RESOLUTION = 10 # 0.5m
...
def get_search_grid_points():
    ...
    for x in range(0, img_width, SEARCH_GRID_RESOLUTION):
        for y in range(0, img_height, SEARCH_GRID_RESOLUTION):
            if map[x, y] == MapValue.FREE.value:
                # each pose: [x,y,theta]
                search_grid_points.append(np.array([x,y,0]))
    return search_grid_points

def visualize_map(map_image, origin_px, best_laser_endbeam_xy=None, p_free_endbeams=None):
  ...
  plt.imshow(map_image, cmap='gray', extent=[x_ticks[0], x_ticks[-1], y_ticks[0], y_ticks[-1]])  # Maps are often best visualized in grayscale
  ...
  def show_scatter_plot(points: List[np.ndarray], color: str, label: str):
    ...
    plt.scatter(x_coords, y_coords, c=color, marker='o', label=label, s=2, linewidths=0.05)
    # Adding a legend to identify the points
    plt.legend()
  if best_laser_endbeam_xy is not None:
      show_scatter_plot(points = best_laser_endbeam_xy, color = 'red', label = 'Laser Endpoints')

  if p_free_endbeams is not None:
      show_scatter_plot(points = p_free_endbeams, color = 'green', label = 'Last Free Points')
...
def transform_laser_scan(laser_scan, pose):
  ...
  laser_beam_endpoints = [
      ...
      ]) for range, bearing in zip(laser_scan, angles) if range < effective_range
  ]
  return laser_beam_endpoints


def _get_vector_1_pixel_away(start, end):
    vec = (end - start)
    norm = np.linalg.norm(vec)
    if norm == 0:
        return np.array([0, 0])  # handle the case where start and end are the same
    return np.round(vec / norm).astype(int)

def _matrix_to_map_pixel(matrix_points, origin_in_matrix):
    pass

def get_laser_endbeam_relative_for_all_thetas(search_thetas: np.ndarray, bearings: np.ndarray, scan_msg: np.ndarray, effective_range: float, resolution: float):
    # return array doesn't have inf. And they are in pixelized map frame
    assert len(scan_msg) == len(bearings), "number of bearings and scan_msg should be the same"
    p_hits_for_all_thetas = []
    # [theta1[(x,y)...], theta2 [...]]
    for theta in search_thetas:
        p_hit_xs_relative = [ np.round(d * np.cos(bearing + theta) / resolution )for d, bearing in zip(scan_msg, bearings) if d <= effective_range] #x = d*cos(theta+bearing)
        p_hit_ys_relative = [ np.round(d * np.sin(bearing + theta) /resolution) for d, bearing in zip(scan_msg, bearings) if d <= effective_range] #y = d*sin(theta+bearing)
        p_hits_for_all_thetas.append(np.asarray([np.array((x,y), dtype=int) for x, y in zip(p_hit_xs_relative, p_hit_ys_relative)])) 
    # combine_xy_for_all_thetas
    return p_hits_for_all_thetas

def get_p_frees_for_all_thetas(search_thetas: np.ndarray, p_hits_for_all_thetas: List[np.ndarray]):
    # [theta1[np.array(x,y)...], theta2 [...]]
    p_free_relative = [] 
    for p_hits_for_theta in p_hits_for_all_thetas:
        unit_vecs = get_vector_1_pixel_away_vectorized(p_hits_for_theta, np.array([0,0]))
        p_frees_for_theta = p_hits_for_theta + unit_vecs
        p_free_relative.append(p_frees_for_theta)

    return p_free_relative

def add_pose_to_relative_poses(p_hits_for_all_thetas: List[np.ndarray], pose: np.ndarray):
    # pose is [x,y]
    for p_hits_for_all_thetas_single in p_hits_for_all_thetas:
...
if __name__ == '__main__':
    ...
    origin_px = (np.array([-map_metadata["origin"][0], -map_metadata["origin"][1]]) / resolution).astype(int)
    ...
    beam_search_kernel = list(range(-BEAM_SEARCH_KERNEL_SIZE, BEAM_SEARCH_KERNEL_SIZE, 1))
    #meter -> pixels
    ...
    # 1. Search grid TODO
    # search_grid_points = get_search_grid_points()
    # search_grid_points_map_pixelized = _matrix_to_map_pixel(search_grid_points, origin_px)
    search_grid_points_map_pixelized =  [np.array([0,0])]

    # TODO
    search_thetas = np.arange(0, 2*np.pi, np.pi/8.0)
    ...
    # [theta1[(x,y)...], theta2 [...]]
    p_hits_for_all_thetas = get_laser_endbeam_relative_for_all_thetas(search_thetas=search_thetas, bearings=bearings, scan_msg=trial_scan_msg, effective_range=effective_range, resolution=resolution)
    p_frees_for_all_thetas = get_p_frees_for_all_thetas(search_thetas, p_hits_for_all_thetas)

    # # optimization
    for pose in search_grid_points_map_pixelized:
        # all map coords thetas
        add_pose_to_relative_poses(p_hits_for_all_thetas, pose[:2])
        add_pose_to_relative_poses(p_frees_for_all_thetas, pose[:2])
        # visualize_map(map_image, origin_px, p_hits_for_all_thetas[0], p_frees_for_all_thetas[0])
        # To matrix coords
        p_hit_in_matrix_coords_for_all_thetas = map_pixel_to_matrix(points_for_all_thetas=p_hits_for_all_thetas, origin_px=origin_px, img_height=img_height)
        p_free_in_matrix_coords_for_all_thetas = map_pixel_to_matrix(points_for_all_thetas=p_frees_for_all_thetas, origin_px=origin_px, img_height=img_height)
        for theta_idx in range(len(search_thetas)):
            ...
            visualize_map(result_map, origin_px, p_hits_for_all_thetas[0], p_frees_for_all_thetas[0])
    ...
    # best_score, best_pose, best_laser_endbeam_xy = optimize_pose(map, all_data[0], np.array([]))
    # visualize_map(map_image, best_pose, best_laser_endbeam_xy)
